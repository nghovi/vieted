...
import type { GeographyQuestionSetProgress, HistoryScorePoint } from "@/lib/db";
import {
  TrendChartDialog,
  TrendChartTrigger,
...
type Props = {
  chapterId: string;
  sets: GeographyQuestionSetProgress[];
};
...
export function GeographyTestSetList({ chapterId, sets }: Props) {
        ...
        {sets.map((set) => (
          <article key={set.setId} className="feature-card">
            ...
            <div className="inline-actions">
              <Link
                href={`/geography/grade-9/${chapterId}/test/${set.setId}`}
