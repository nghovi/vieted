...
export function GeographyChapterForm({ initialChapterId, chapters }: Props) {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch("/api/student/geography-chapter", {
      method: "POST",
    ...
    if (!response.ok) {
      setMessage(data.error ?? "Không thể lưu chương học.");
      setIsSaving(false);
      return;
    }

    setMessage("Đã lưu chương học hiện tại.");
    setIsSaving(false);
    router.refresh();
  }
