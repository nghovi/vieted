...
import type { HistoryScorePoint, MathQuestionSetAttemptResult } from "@/lib/db";
...
export function MathTestForm({
  ...
  const [result, setResult] = useState<MathQuestionSetAttemptResult | null>(null);
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch("/api/math/grade-9/test/submit", {
      method: "POST",
    ...
    const data = (await response.json()) as {
      error?: string;
      result?: MathQuestionSetAttemptResult;
    };
