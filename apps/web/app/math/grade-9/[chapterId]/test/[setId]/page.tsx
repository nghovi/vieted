...
import {
  getLatestMathQuestionSetAttempt,
  getMathQuestionSetByIdFromDb,
  listMathQuestionSetsWithProgress,
} from "@/lib/db";
import { MathTestForm } from "../math-test-form";

type Props = {
  params: Promise<{ chapterId: string; setId: string }>;
};

export default async function MathSetAttemptPage({ params }: Props) {
  ...
  const latestAttempt = await getLatestMathQuestionSetAttempt(student.dbId, chapterId, setId);
  const chapterProgress = await listMathQuestionSetsWithProgress(student.dbId, chapterId);
    ...
    <main className="page-shell">
      <section className="hero">
        <div className="hero-copy">
          ...
          <p className="lede">
            Hoàn thành từng câu hỏi rồi nộp bài để xem đáp án, giải thích và điểm số.
          </p>
