...
export default async function MathChapterPage({ params }: Props) {
  const { chapterId } = await params;
  const student = await getServerSessionStudent();
  const chapter = await getMathChapterByIdFromDb(chapterId);
  const evaluation = student ? await getMathChapterEvaluation(student.dbId, chapterId) : null;
    ...
    <main className="page-shell">
      <section className="hero">
        <div className="hero-copy">
          <p className="eyebrow">Toán lớp 9</p>
          <h1>{chapter.title}</h1>
